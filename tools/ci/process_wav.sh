...
help()
{
   ...
   echo
   echo "Syntax: process_wav.sh [-h|c|d] to_device.wav from_device.wav"
   echo "options:"
   echo "h     Print this Help."
   echo "c     Number of channels in input wav"
   echo "d     Card Device Name e.g.: 'hw:CARD=Avona Voice Reference Design,DEV=0'"
   echo
}
...
DEVICE_DEFAULT="hw:CARD=Design,DEV=0"

# determine driver & device
uname=`uname`
if [[ "$uname" == 'Linux' ]]; then
    DEVICE_DRIVER="alsa"
    DEVICE_NAME="${DEVICE_OPT:=$DEVICE_DEFAULT}"
